...
### 2. ローカル CLI（Claude Code / Codex / Antigravity）
...
セットアップ（macOS）:
1. 対象 CLI を導入・ログイン（`claude` / `codex` / `agy`(Antigravity)）
...
- CLI ごとに**モデルを選択**できます（設定の「モデル」。空＝各 CLI の既定）。
  内部では claude=`--model`、codex=`-m`、antigravity(agy)=`--model` を付与します。
...
- **ツール無効/読み取り専用で起動**: claude=`--disallowed-tools`（Bash/Read/Write/WebFetch 等を禁止）、
  antigravity=`--sandbox`（端末制限。`--dangerously-skip-permissions` は付けない）、codex=`--sandbox read-only`
