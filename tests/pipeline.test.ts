...
describe('assembleFinalSummary の tldr 解析 (optional, CLI 限定)', () => {
  function byOrdinal(...ordinals: number[]): Map<number, ClusterComment> {
    return new Map(
      ordinals.map((o) => [
        o,
  ...
  it('無ければ undefined', () => {
    const summary = assembleFinalSummary(base, page, 'ja', byOrdinal(1))
    expect(summary.tldr).toBeUndefined()
  })
  ...
  it('正常系: problem / solution を採用する', () => {
    ...
    expect(summary.tldr?.problem).toBe('ビルドが遅く CI が 30 分かかる')
    expect(summary.tldr?.solution).toBe('依存を整理しキャッシュを有効化する')
  })
  ...
  it('長すぎる文は 300 字に切り詰める', () => {
    const summary = assembleFinalSummary(
      {
        ...base,
        tldr: { problem: 'あ'.repeat(400), solution: 'S' },
      },
      page,
      'ja',
      byOrdinal(1),
    )
    expect(summary.tldr?.problem).toBe('あ'.repeat(300))
  })
})
...
describe('スキーマの status フィールド (Nano 安定性の保護)', () => {
  ...
  it('FINAL_SCHEMA に tldr は無く、WITH_FLOW にはあり required ではない', () => {
    const props = FINAL_SCHEMA.properties as Record<string, unknown>
    expect(props).not.toHaveProperty('tldr')
    const flowProps = FINAL_SCHEMA_WITH_FLOW.properties as Record<
      string,
      unknown
    >
    expect(flowProps).toHaveProperty('tldr')
    expect(FINAL_SCHEMA_WITH_FLOW.required).not.toContain('tldr')
  })
